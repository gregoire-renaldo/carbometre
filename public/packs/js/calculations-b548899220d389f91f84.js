...
function total() {
  ...
  var numberDivers = Number(totalDivers.textContent);
  var numberValeur = Number(totalValeur.textContent);
  var sumTotal = numberVoiture + numberAlim + numberTransport + numberEnergie + numberAutre + numberValeur + numberVetements + numberDivers;
...
function totEnergie() {
  ...
  var sumEnergie = numberDechet + numberElectricite + numberGazkwh + numberGazm + numberFioul + numberGranulebois + numberBuchebois + numberEau + numberEauusee;
...
function multiplyGazkwh() {
  var one = parseFloat(userGazkwh.value) || 0;
...
var defaultEnergie = (Number(defaultElectricite) + Number(defaultGazkwh) + Number(defaultGazm) + Number(defaultFioul) + Number(defaultGranulebois) + Number(defaultBuchebois) + Number(defaultEau)).toFixed(2);
...
function totVetements() {
  ...
  var numberChemiseViscose = Number(scoreChemisecviscose.textContent);
  var numberTshirtCoton = Number(scoreTshirtCoton.textContent);
  var numberTshirtPoly = Number(scoreTshirtPolyester.textContent);
  var numberPolair = Number(scorePolair.textContent);
  var numberPullAcry = Number(scorePullAcrylique.textContent);
  var numberPulllaine = Number(scorePullEnLaine.textContent);
  var numberSweat = Number(scoreSweat.textContent);
  var numberPolo = Number(scorePolo.textContent);
  var numberRobeC = Number(scoreRobeCoton.textContent);
  var numberRobePoly = Number(scoreRobePoly.textContent);
  var numberChaussureTissu = Number(scoreChaussureTissu.textContent);
  var numberSport = Number(scoreChaussureSport.textContent);
  var numberAnorak = Number(scoreAnorak.textContent);
  var sum = numberChemiseCoton + numberChemiseViscose + numberTshirtCoton + numberTshirtPoly + numberPolair + numberPullAcry + numberPulllaine + numberSweat + numberPolo + numberRobeC + numberRobePoly + numberChaussureTissu + numberSport + numberAnorak + numberJeanmonde + numberJeanfr;
...
var userChemisecviscose = document.getElementById('userChemisecviscose');
var chemiseviscosecc = 10;
var scoreChemisecviscose = document.getElementById('scoreChemisecviscose');
var defaultChemiseviscose = (userChemisecviscose.value * chemiseviscosecc).toFixed(2);
...
function multiplyChemiseViscose() {
  var one = parseFloat(userChemisecviscose.value) || 0;
  var cc = parseFloat(one * chemiseviscosecc).toFixed(2);
  scoreChemisecviscose.innerHTML = cc;
  totVetements();
} // field t shirt coton


var userTshirtCoton = document.getElementById('userTshirtCoton');
var tshirtcotoncc = 5;
var scoreTshirtCoton = document.getElementById('scoreTshirtCoton');
var defaultTshirtCoton = (userTshirtCoton.value * tshirtcotoncc).toFixed(2);
...
function multiplyTshirtCoton() {
  var one = parseFloat(userTshirtCoton.value) || 0;
  var cc = parseFloat(one * tshirtcotoncc).toFixed(2);
  scoreTshirtCoton.innerHTML = cc;
  totVetements();
} // field t shirt poly


var userTshirtPolyester = document.getElementById('userTshirtPolyester');
var tshirtpolycc = 6;
var scoreTshirtPolyester = document.getElementById('scoreTshirtPolyester');
var defaultTshirtPoly = (userTshirtPolyester.value * tshirtpolycc).toFixed(2);
...
function multiplyTshirtPoly() {
  var one = parseFloat(userTshirtPolyester.value) || 0;
  var cc = parseFloat(one * tshirtpolycc).toFixed(2);
  scoreTshirtPolyester.innerHTML = cc;
  totVetements();
} // field polair


var userPolair = document.getElementById('userPolair');
var polaircc = 24;
var scorePolair = document.getElementById('scorePolair');
var defaultPolair = (userPolair.value * polaircc).toFixed(2);
scorePolair.innerHTML = defaultPolair;
userPolair.addEventListener("input", multiplyTPolair);

function multiplyTPolair() {
  var one = parseFloat(userPolair.value) || 0;
  var cc = parseFloat(one * polaircc).toFixed(2);
  scorePolair.innerHTML = cc;
  totVetements();
} // field pull acrylique


var userPullAcrylique = document.getElementById('userPullAcrylique');
var pullacryliquecc = 26;
var scorePullAcrylique = document.getElementById('scorePullAcrylique');
var defaultAcrylique = (userPullAcrylique.value * pullacryliquecc).toFixed(2);
...
function multiplyPullAcry() {
  var one = parseFloat(userPullAcrylique.value) || 0;
  var cc = parseFloat(one * pullacryliquecc).toFixed(2);
  scorePullAcrylique.innerHTML = cc;
  totVetements();
} // field pull laine


var userPullEnLaine = document.getElementById('userPullEnLaine');
var pullenlainecc = 53;
var scorePullEnLaine = document.getElementById('scorePullEnLaine');
var defaultpullenlaine = (userPullEnLaine.value * pullenlainecc).toFixed(2);
...
function multiplyPulllaine() {
  var one = parseFloat(userPullEnLaine.value) || 0;
  var cc = parseFloat(one * pullenlainecc).toFixed(2);
  scorePullEnLaine.innerHTML = cc;
  totVetements();
} // field sweat


var userSweat = document.getElementById('userSweat');
var sweatcc = 27;
var scoreSweat = document.getElementById('scoreSweat');
var defaultsweat = (userSweat.value * sweatcc).toFixed(2);
scoreSweat.innerHTML = defaultsweat;
userSweat.addEventListener("input", multiplySweat);

function multiplySweat() {
  var one = parseFloat(userSweat.value) || 0;
  var cc = parseFloat(one * sweatcc).toFixed(2);
  scoreSweat.innerHTML = cc;
  totVetements();
} // field polo


var userPolo = document.getElementById('userPolo');
var polocc = 8;
var scorePolo = document.getElementById('scorePolo');
var defaultpolo = (userPolo.value * polocc).toFixed(2);
scorePolo.innerHTML = defaultpolo;
userPolo.addEventListener("input", multiplyPolo);

function multiplyPolo() {
  var one = parseFloat(userPolo.value) || 0;
  var cc = parseFloat(one * polocc).toFixed(2);
  scorePolo.innerHTML = cc;
  totVetements();
} // field robe coton


var userRobeCoton = document.getElementById('userRobeCoton');
var robecotoncc = 50;
var scoreRobeCoton = document.getElementById('scoreRobeCoton');
var defaultRobeCoton = (userRobeCoton.value * robecotoncc).toFixed(2);
scoreRobeCoton.innerHTML = defaultRobeCoton;
userRobeCoton.addEventListener("input", multiplyRobeC);

function multiplyRobeC() {
  var one = parseFloat(userRobeCoton.value) || 0;
  var cc = parseFloat(one * robecotoncc).toFixed(2);
  scoreRobeCoton.innerHTML = cc;
  totVetements();
} // field robe poly


var userRobePoly = document.getElementById('userRobePoly');
var robecpolycc = 52;
var scoreRobePoly = document.getElementById('scoreRobePoly');
var defaultRobePoly = (userRobePoly.value * robecpolycc).toFixed(2);
scoreRobePoly.innerHTML = defaultRobePoly;
userRobePoly.addEventListener("input", multiplyRobePoly);

function multiplyRobePoly() {
  var one = parseFloat(userRobePoly.value) || 0;
  var cc = parseFloat(one * robecpolycc).toFixed(2);
  scoreRobePoly.innerHTML = cc;
  totVetements();
} // field chaussure tissu


var userChaussureTissu = document.getElementById('userChaussureTissu');
var chaussureTissucc = 17;
var scoreChaussureTissu = document.getElementById('scoreChaussureTissu');
var defaultChaussureTissu = (userChaussureTissu.value * chaussureTissucc).toFixed(2);
...
function multiplyChaussureTissu() {
  var one = parseFloat(userChaussureTissu.value) || 0;
  var cc = parseFloat(one * chaussureTissucc).toFixed(2);
  scoreChaussureTissu.innerHTML = cc;
  totVetements();
} // field chaussure sport


var userChaussureSport = document.getElementById('userChaussureSport');
var chaussureSportcc = 19;
var scoreChaussureSport = document.getElementById('scoreChaussureSport');
var defaultChaussureSport = (userChaussureSport.value * chaussureSportcc).toFixed(2);
...
function multiplyChaussureSport() {
  var one = parseFloat(userChaussureSport.value) || 0;
  var cc = parseFloat(one * chaussureSportcc).toFixed(2);
  scoreChaussureSport.innerHTML = cc;
  totVetements();
} // field anorak


var userAnorak = document.getElementById('userAnorak');
var anorakcc = 39;
var scoreAnorak = document.getElementById('scoreAnorak');
var defaultAnorak = (userAnorak.value * anorakcc).toFixed(2);
scoreAnorak.innerHTML = defaultAnorak;
userAnorak.addEventListener("input", multiplyAnorak);

function multiplyAnorak() {
  var one = parseFloat(userAnorak.value) || 0;
  var cc = parseFloat(one * anorakcc).toFixed(2);
  scoreAnorak.innerHTML = cc;
  totVetements();
}

var defaultVetement = (Number(defaultChemiseCoton) + Number(defaultChemiseviscose) + Number(defaultTshirtCoton) + Number(defaultTshirtPoly) + Number(defaultPolair) + Number(defaultAcrylique) + Number(defaultpullenlaine) + Number(defaultsweat) + Number(defaultpolo) + Number(defaultRobeCoton) + Number(defaultRobePoly) + Number(defaultChaussureTissu) + Number(defaultChaussureSport) + Number(defaultAnorak) + Number(defaultJeanmonde) + Number(defaultJeanfr)).toFixed(2);
totalVetement.innerHTML = defaultVetement; // category autre
...
function totAutre() {
  ...
  var numberVoitElecMoins = Number(scoreVoitureElecMoins.textContent);
  var numberVoitElecPlus = Number(scoreVoitureElecPlus.textContent);
  var numberTroti = Number(scoreTrotinette.textContent);
  var numberAppartementneuf = Number(scoreAppartementneuf.textContent);
  var sum = numberAspirateur + numberCongelateur + numberRefrigerateur + numberFourelec + numberLavelinge + numberMicroonde + numberSmartphone + numberOrdinateur + numberAutofr + numberVelo + numberVeloelec + numberAutofrelec + numberAppartementneuf + numberAutoasie + numberAutoasieelec + numberVoitElecMoins + numberVoitElecPlus + numberTroti;
...
var userVoitureElecMoins = document.getElementById('userVoitureElecMoins');
var voitelecmoinscc = 11000;
var scoreVoitureElecMoins = document.getElementById('scoreVoitureElecMoins');
var defaultVoitElecMoins = (userVoitureElecMoins.value * voitelecmoinscc).toFixed(2);
...
function multiplyVoitElecMoins() {
  var one = parseFloat(userVoitureElecMoins.value) || 0;
  var cc = parseFloat(one * voitelecmoinscc).toFixed(2);
  scoreVoitureElecMoins.innerHTML = cc;
  totAutre();
} // field voiture electrique > 50kwh


var userVoitureElecPlus = document.getElementById('userVoitureElecPlus');
var voitelecpluscc = 14500;
var scoreVoitureElecPlus = document.getElementById('scoreVoitureElecPlus');
var defaultVoitElecPlus = (userVoitureElecPlus.value * voitelecpluscc).toFixed(2);
...
function multiplyVoitElecPlus() {
  var one = parseFloat(userVoitureElecPlus.value) || 0;
  var cc = parseFloat(one * voitelecpluscc).toFixed(2);
  scoreVoitureElecPlus.innerHTML = cc;
  totAutre();
} // field troti


var userTrotinette = document.getElementById('userTrotinette');
var troticc = 92;
var scoreTrotinette = document.getElementById('scoreTrotinette');
var defaultTroti = (userTrotinette.value * troticc).toFixed(2);
scoreTrotinette.innerHTML = defaultTroti;
userTrotinette.addEventListener("input", multiplyTroti);

function multiplyTroti() {
  var one = parseFloat(userTrotinette.value) || 0;
  var cc = parseFloat(one * troticc).toFixed(2);
...
var defaultAutre = (Number(defaultAspirateur) + Number(defaultCongelateur) + Number(defaultRefrigerateur) + Number(defaultFourelec) + Number(defaultLavelinge) + Number(defaultMicroonde) + Number(defaultSmartphone) + Number(defaultOrdinateur) + Number(defaultVelo) + Number(defaultVeloelectrique) + Number(defaultAutofr) + Number(defaultAutofrelec) + Number(defaultAutoasie) + Number(defaultAutoasieelec) + Number(defaultAppartementneuf) + Number(defaultVoitElecMoins) + Number(defaultVoitElecPlus) + Number(defaultTroti)).toFixed(2);
totalAutre.innerHTML = defaultAutre; // category divers

var totalDivers = document.getElementById('totalDivers');
...
function totDivers() {
  ...
  var sum = numberStreaming + numberTweet + numberMail;
...
var defaultDivers = (+Number(defaultStreaming) + Number(defaultTweet) + Number(defaultMail)).toFixed(2);
totalDivers.innerHTML = defaultDivers; // category valeur additionnelle
...
totalBilan.innerHTML = (Number(defaultVoiture) + Number(defaultTransport) + Number(defaultEnergie) + Number(defaultAlim) + Number(defaultAutre) + Number(defaultValeur) + Number(defaultDivers)).toFixed(2);
...
/******/ });
//# sourceMappingURL=calculations-b548899220d389f91f84.js.map